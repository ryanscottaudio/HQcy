...
class QuestionInput: UIView {
    ...
    @IBOutlet weak var correctLabel: Popup!
    @IBOutlet weak var wrongLabel: Popup!
    ...
    func askQuestion(text: String, options: [String], onAnswer: @escaping (Int) -> Void) {
        ...
        preSetup()
        postSetup(text: text, options: options, votes: nil, correctAnswer: nil)
    }
    ...
    func showAnswer(votes: [Int], correctAnswer: Int) {
        ...
        postSetup(text: text!, options: options!, votes: votes, correctAnswer: correctAnswer)
    }
    ...
    func preSetup() {
        ...
        correctLabel.backgroundColor = correctColor
        wrongLabel.backgroundColor = wrongColor
    ...
    func postSetup(text: String, options: [String], votes: [Int]?, correctAnswer: Int?) {
        ...
        let labelToShow = isShowingAnswer
            ? chosenAnswer == correctAnswer
                ? correctLabel
                : wrongLabel
            : timerLabel
        
        let safeGuide = superview
        ...
        labelToShow.transform = CGAffineTransform(scaleX: 0, y: 0)
        ...
        CATransaction.setCompletionBlock {
            if labelToShow == self.timerLabel {
        ...
        UIView.animate(withDuration: animationDuration) {
            ...
            labelToShow.transform = CGAffineTransform(scaleX: 1, y: 1)
        }
        
        CATransaction.commit()
        
        if chosenAnswer == correctAnswer {
            playSound(resourceName: "ka-ching")
        }
